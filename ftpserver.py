...
class FTPserverThread(threading.Thread):
    def __init__(self,(conn,addr), drive):
        self.conn=conn
        self.addr=addr
        self.basewd=currdir
    ...
    def LIST(self,cmd):
        ...
        print 'list:', self.cwd
        found_files = self.drive.listFiles()
        ...
        for file in found_files:
          ...
          k='{0}rw-rw-rw- 1 user group {1} {2} {3}'.format(d, file.size, ftime, file.name)
...
class FTPserver(threading.Thread):
    ...
    def run(self):
        self.sock.listen(5)
        while True:
            th=FTPserverThread(self.sock.accept(), self.drive)
            th.daemon=True
            th.start()
